...
from keras.layers import (ZeroPadding2D, Dropout, Convolution2D, Flatten, Dense,
...
from caffe_utils import (layer_type, normalize_layers,
                                     get_output_names, is_data_input)
from extra_layers import Select
...
def handle_concat(spec, bottoms):
    ...
    return merge(concat_axis=axis, name=spec.name)(bottoms)
...
def load_weights(model, weights):
    for layer in model.layers:
        ...
        # trainable weights
        if layer.name in weights and weights[layer.name][0].shape==model.get_layer(layer.name).get_weights()[0].shape:
